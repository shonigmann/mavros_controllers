...
geometricCtrl::geometricCtrl(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private):
  ...
  //Note - added "_" to "mavname" to match launch file...
  nh_private_.param<string>("mav_name", mav_name_, "iris");
...
void geometricCtrl::computeBodyRateCmd(Eigen::Vector4d &bodyrate_cmd){
  ...
  //note2: used to be MAVVel in arc tan function
  if(velocity_yaw_) {
    mavYaw_ = std::atan2(-1.0 * targetVel_(1), targetVel_(0));
  } else{
    mavYaw_ = targetYaw_;
  }
